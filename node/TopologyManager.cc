...
void TopologyManager::handleMessage(cMessage *msg)
{
    ...
    for (int i = 0; i < topo->getNumNodes(); i++) {
        ...
        EV << "src node: " << srcN->getModule()->getIndex() << " num out links: " << numOutLinks << endl;

        for (int j = 0; j < numOutLinks; j++) {

            if (true) {
                EV << "src node: " << srcN->getModule()->getIndex() << "<->" << " local gate id: " << srcN->getLinkOut(j)->getLocalGate()->getConnectionId() << "<->" << " dst: " << srcN->getLinkOut(j)->getRemoteNode()->getModule()->getIndex()<<endl;
    ...
    EV << "src: " << src << " dest: " << dst << endl;
    for (int idx = 0; idx < nodeDis.size(); idx++) {
        EV << "distance: " << nodeDis[idx] << endl;
    }
